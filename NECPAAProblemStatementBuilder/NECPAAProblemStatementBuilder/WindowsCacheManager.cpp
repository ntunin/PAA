...
WindowsCacheManager::WindowsCacheManager(std::string path, std::string subdir) {
	this->path = path;
	this->subdir = subdir;
...
void WindowsCacheManager::fill(std::string filename, NecOut **outs) {
	std::string path = cachedFileName(subdir);
	std::vector<std::string> files = WindowsUtils::contentsOfDirectory(path);
	int count = files.size();
	for (int i = 0; i < count; i++) {
		std::string path = cachedFileName(subdir).append("/").append(files[i]);
...
void WindowsCacheManager::cache(std::string filename) {
	std::string path = cachedFileName(subdir);
	WindowsUtils::createDirectory(path);
